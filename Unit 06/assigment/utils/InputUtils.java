...
public class InputUtils {
     ...
     * If the input is invalid or outside the range, the user is prompted to try
     * 
     * @param message The message to display to the user when asking for input.
    ...
    public static int getInt(String message, int min, int max){
        ...
        if (input < min || input > max) {
            PrintUtils.printFail("Input must be between " + min + " and " + max);
            return getInt(message, min, max); // Recursively ask for input until valid
        }
        return input;
    }
    ...
    public static double getDouble(String message, double min, double max) {
        ...
        double input = getDouble(message, true);

        if (input < min || input > max) {
            PrintUtils.printFail("Input must be between " + min + " and " + max);
            return getDouble(message, min, max); // Recursively ask for input until valid
        }
        return input;
    }
